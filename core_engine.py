...
class ClaimsEngine:
    ...
    def _seed_demo_if_empty(self):
        ...
        self.db.executemany("""
            INSERT INTO claims (claim_ref, policy_ref, claimant_email, status, status_changed_at, created_at, updated_at, incident_date, incident_type, incident_location, incident_description, estimated_amount, claim_class, fast_track, total_loss_indicator, assigned_to, assigned_role, external_ref, salvage_value, discharge_voucher_signed, discharge_voucher_date, appeal_filed, appeal_ref, extra_data)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, claims_data)
        self.db.commit()
        reserves = [
